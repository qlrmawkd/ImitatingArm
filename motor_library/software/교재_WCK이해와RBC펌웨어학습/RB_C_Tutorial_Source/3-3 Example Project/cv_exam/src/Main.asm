...
;      17 
;      18 // Flag ----------------------------------------------------------------------------------------
;      19 bit 	F_PLAYING;			// Show the motion running Ã;      20 bit 	F_DIRECT_C_EN;		        // wCK direct control mode (1:Available, 0:Not available)
;      21 
;      22 // Button Input----------------------------------------------------------------------------------
;      23 WORD    gBtnCnt;			// Button press process counterÍ;      24 
;      25 // Time Measurement ------------------------------------------------------------------------------
;      26 WORD    gMSEC;
;      27 BYTE    gSEC;
;      28 BYTE    gMIN;
...
;      35 BYTE	gTx0BufIdx;			// UART0 transmission buffer indexº;      36 char	gRx0Buf[RX0_BUF_SIZE];		// UART0 transmission buffer 
_gRx0Buf:
	.BYTE 0x8
...
;     388 	#asm("sei");
	sei
;     389 	TIMSK |= 0x01;		// Timer0 Overflow Interrupt È°¼ºÈ­
	IN   R30,0x37
	ORI  R30,1
	OUT  0x37,R30
;     390 
;     391 	SpecialMode();
	CALL _SpecialMode
;     392     
;     393     SendSetCmd(1, ID_SET, 10, 10);  // ID 1¹ø wCkÀÇ ID¸¦ 10À¸·Î º¯°æ
	LDI  R30,LOW(1)
	ST   -Y,R30
	LDI  R30,LOW(12)
	ST   -Y,R30
...
;     396 		/*
;     397 		lMSEC = gMSEC;
;     398 		ReadButton();	    // ¹öÆ° ÀÐ±â
;     399 		IoUpdate();		    // IO »óÅÂ UPDATE
;     400 		while(lMSEC==gMSEC);
;     401 		*/
;     402 		PositionMove(1, 1, 200);        // ÅäÅ©1, ID1, ¸ñÇ¥À§Ä¡ 200 
	LDI  R30,LOW(1)
	ST   -Y,R30
	ST   -Y,R30
	LDI  R30,LOW(200)
	ST   -Y,R30
	CALL _PositionMove
;     403 		IOwrite( 1, 0);                 // ID 1, 0¹ø LED ON
	LDI  R30,LOW(1)
	ST   -Y,R30
	LDI  R30,LOW(0)
	ST   -Y,R30
...
;     405 		PWR_LED2_OFF;    delay_ms(500);                             		
	...
	CALL _delay_ms
;     406 
;     407 		PositionMove(1, 1, 50);        // ÅäÅ©1, ID1, ¸ñÇ¥À§Ä¡ 200 
	LDI  R30,LOW(1)
	ST   -Y,R30
	ST   -Y,R30
	LDI  R30,LOW(50)
	ST   -Y,R30
	CALL _PositionMove
;     408 		IOwrite( 1, 3);                 // ID 1, 0¹ø, 1¹ø LED ON
	LDI  R30,LOW(1)
	ST   -Y,R30
	LDI  R30,LOW(3)
	ST   -Y,R30
...
;     471 
;     472 //------------------------------------------------------------------------------
;     473 // 8bit ¸í·É Position Move¸¦ ¼öÇàÇÏ±â À§ÇÑ ÇÔ¼ö
...
;     489 
;     490 //------------------------------------------------------------------------------
;     491 // È®Àå ¸í·É I/O Write¸¦ ¼öÇàÇÏ±â À§ÇÑ ÇÔ¼ö
...
;     500 	CheckSum = (ID^100^IOchannel^IOchannel)&0x7f;
	...
	MOV  R16,R30
;     501 
;     502 	gTx0Buf[gTx0Cnt]=HEADER;	gTx0Cnt++;			// ¼Û½ÅÇÒ ¹ÙÀÌÆ®¼ö Áõ°¡
	MOV  R26,R11
	LDI  R27,0
	...
	ST   X,R30
	INC  R11
;     503 	gTx0Buf[gTx0Cnt]=ID;    	gTx0Cnt++;			// ¼Û½ÅÇÒ ¹ÙÀÌÆ®¼ö Áõ°¡
	MOV  R30,R11
	LDI  R31,0
	SUBI R30,LOW(-_gTx0Buf)
	SBCI R31,HIGH(-_gTx0Buf)
	...
	STD  Z+0,R26
	INC  R11
;     504 	gTx0Buf[gTx0Cnt]=100;    	gTx0Cnt++;			// ¼Û½ÅÇÒ ¹ÙÀÌÆ®¼ö Áõ°¡
	MOV  R26,R11
	LDI  R27,0
	SUBI R26,LOW(-_gTx0Buf)
	SBCI R27,HIGH(-_gTx0Buf)
	...
	ST   X,R30
	INC  R11
;     505 	gTx0Buf[gTx0Cnt]=IOchannel;	gTx0Cnt++;			// ¼Û½ÅÇÒ ¹ÙÀÌÆ®¼ö Áõ°¡
	MOV  R30,R11
	LDI  R31,0
	SUBI R30,LOW(-_gTx0Buf)
	SBCI R31,HIGH(-_gTx0Buf)
	...
	STD  Z+0,R26
	INC  R11
;     506 	gTx0Buf[gTx0Cnt]=IOchannel; gTx0Cnt++;			// ¼Û½ÅÇÒ ¹ÙÀÌÆ®¼ö Áõ°¡
	MOV  R30,R11
	LDI  R31,0
	SUBI R30,LOW(-_gTx0Buf)
	SBCI R31,HIGH(-_gTx0Buf)
	STD  Z+0,R26
	INC  R11
;     507 	gTx0Buf[gTx0Cnt]=CheckSum;	gTx0Cnt++;			// ¼Û½ÅÇÒ ¹ÙÀÌÆ®¼ö Áõ°¡    
	MOV  R26,R11
	LDI  R27,0
	SUBI R26,LOW(-_gTx0Buf)
	SBCI R27,HIGH(-_gTx0Buf)
...
;     509 	gTx0BufIdx++;
	INC  R13
;     510 	sciTx0Data(gTx0Buf[gTx0BufIdx-1]);		// Ã¹¹ÙÀÌÆ® ¼Û½Å ½ÃÀÛ
	MOV  R30,R13
	SUBI R30,LOW(1)
	LDI  R31,0
	SUBI R30,LOW(-_gTx0Buf)
...
;     544 
;     545 //------------------------------------------------------------------------------
;     546 // µ¿±âÈ­ À§Ä¡ ¸í·É(Synchronized Position Send Command)À» º¸³»´Â ÇÔ¼ö
;     547 //------------------------------------------------------------------------------
...
;     562 	gTx0Buf[gTx0Cnt]=16;
	...
	LDI  R30,LOW(16)
	ST   X,R30
;     563 	gTx0Cnt++;			// ¼Û½ÅÇÒ ¹ÙÀÌÆ®¼ö Áõ°¡
	INC  R11
;     564 
;     565 	CheckSum = 0;
	LDI  R18,LOW(0)
;     566 	for(i=0;i<MAX_wCK;i++){	// °¢ wCK µ¥ÀÌÅÍ ÁØºñ
	LDI  R19,LOW(0)
_0x2E:
	CPI  R19,31
	BRLO PC+3
	JMP _0x2F
;     567 		if(Scene.wCK[i].Exist){	// Á¸ÀçÇÏ´Â ID¸¸ ÁØºñ
	LDI  R30,LOW(5)
	MUL  R30,R19
	MOVW R30,R0
...
;     583 
;     584 //------------------------------------------------------------------------------
;     585 // À§Ä¡ ÀÐ±â ¸í·É(Position Send Command)À» º¸³»´Â ÇÔ¼ö
;     586 // Input	: ID, SpeedLevel, Position
;     587 // Output	: Current
;     588 // UART0 RX ÀÎÅÍ·´Æ®, Timer0 ÀÎÅÍ·´Æ®°¡ È°¼ºÈ­ µÇ¾î ÀÖ¾î¾ß ÇÔ
;     589 //------------------------------------------------------------------------------
...
;	startT -> Y+5
;     597 	Data2 = 0;
;     598 	gRx0Cnt = 0;			// ¼ö½Å ¹ÙÀÌÆ® ¼ö ÃÊ±âÈ­
...
;     604 	startT = gMSEC;
;     605 	while(gRx0Cnt<2){
;     606         if(gMSEC<startT){ 	// ¹Ð¸®ÃÊ Ä«¿îÆ®°¡ ¸®¼ÂµÈ °æ¿ì
;     607             if((1000 - startT + gMSEC)>RX_T_OUT)
;     608             	return 444;	// Å¸ÀÓ¾Æ¿ô½Ã ·ÎÄÃ Å»Ãâ
;     609         }
...
;     615 
;     616 //------------------------------------------------------------------------------
;     617 // Flash¿¡¼­ ¸ð¼Ç Á¤º¸ ÀÐ±â
;     618 //	MRIdx : ¸ð¼Ç »ó´ë ÀÎµ¦½º
;     619 //------------------------------------------------------------------------------
;     620 void GetMotionFromFlash(void)
;     621 {
;     622 	WORD i;
;     623 
;     624 	for(i=0;i<MAX_wCK;i++){				// wCK ÆÄ¶ó¹ÌÅÍ ±¸Á¶Ã¼ ÃÊ±âÈ­
;	i -> R16,R17
...
;     631 	}
;     632 	for(i=0;i<Motion.NumOfwCK;i++){		// °¢ wCK ÆÄ¶ó¹ÌÅÍ ºÒ·¯¿À±â
...
;     642 
;     643 //------------------------------------------------------------------------------
;     644 // Runtime P,D,I ÀÌµæ ¼Û½Å
;     645 // 		: ¸ð¼ÇÁ¤º¸¿¡¼­ Runtime P,D,IÀÌµæÀ» ºÒ·¯¿Í¼­ wCK¿¡°Ô º¸³½´Ù
;     646 //------------------------------------------------------------------------------
;     647 void SendTGain(void)
;     648 {
;     649 	WORD i;
;     650 
;     651 	UCSR0B &= 0x7F;   		// UART0 RxÀÎÅÍ·´Æ® ¹Ì»ç¿ë
;	i -> R16,R17
;     652 	UCSR0B |= 0x40;   		// UART0 TxÀÎÅÍ·´Æ® »ç¿ë
;     653 
;     654 	while(gTx0Cnt);			// ÀÌÀü ÆÐÅ¶ ¼Û½ÅÀÌ ³¡³¯ ¶§±îÁö ´ë±â
;     655 	for(i=0;i<MAX_wCK;i++){					// Runtime P,DÀÌµæ ¼³Á¤ ÆÐÅ¶ ÁØºñ
;     656 		if(Motion.wCK[i].Exist)				// Á¸ÀçÇÏ´Â ID¸¸ ÁØºñ
;     657 			SendSetCmd(i, 11, Motion.wCK[i].RPgain, Motion.wCK[i].RDgain);
;     658 	}
;     659 	gTx0BufIdx++;
;     660 	sciTx0Data(gTx0Buf[gTx0BufIdx-1]);		// Ã¹¹ÙÀÌÆ® ¼Û½Å ½ÃÀÛ
;     661 
;     662 
;     663 	while(gTx0Cnt);			// ÀÌÀü ÆÐÅ¶ ¼Û½ÅÀÌ ³¡³¯ ¶§±îÁö ´ë±â
;     664 	for(i=0;i<MAX_wCK;i++){					// Runtime IÀÌµæ ¼³Á¤ ÆÐÅ¶ ÁØºñ
;     665 		if(Motion.wCK[i].Exist)				// Á¸ÀçÇÏ´Â ID¸¸ ÁØºñ
;     666 			SendSetCmd(i, 24, Motion.wCK[i].RIgain, Motion.wCK[i].RIgain);
;     667 	}
;     668 	gTx0BufIdx++;
;     669 	sciTx0Data(gTx0Buf[gTx0BufIdx-1]);		// Ã¹¹ÙÀÌÆ® ¼Û½Å ½ÃÀÛ
;     670 }
;     671 
;     672 
;     673 //------------------------------------------------------------------------------
;     674 // È®Àå Æ÷Æ®°ª ¼Û½Å
;     675 // 		: ¾À Á¤º¸¿¡¼­ È®Àå Æ÷Æ®°ªÀ» ºÒ·¯¿Í¼­ wCK¿¡°Ô º¸³½´Ù
;     676 //------------------------------------------------------------------------------
;     677 void SendExPortD(void)
;     678 {
;     679 	WORD i;
;     680 
;     681 	UCSR0B &= 0x7F;   		// UART0 RxÀÎÅÍ·´Æ® ¹Ì»ç¿ë
;	i -> R16,R17
;     682 	UCSR0B |= 0x40;   		// UART0 TxÀÎÅÍ·´Æ® »ç¿ë
;     683 
;     684 	while(gTx0Cnt);			// ÀÌÀü ÆÐÅ¶ ¼Û½ÅÀÌ ³¡³¯ ¶§±îÁö ´ë±â
;     685 	for(i=0;i<MAX_wCK;i++){					// Runtime P,DÀÌµæ ¼³Á¤ ÆÐÅ¶ ÁØºñ
;     686 		if(Scene.wCK[i].Exist)				// Á¸ÀçÇÏ´Â ID¸¸ ÁØºñ
;     687 			SendSetCmd(i, 100, Scene.wCK[i].ExPortD, Scene.wCK[i].ExPortD);
;     688 	}
;     689 	gTx0BufIdx++;
;     690 	sciTx0Data(gTx0Buf[gTx0BufIdx-1]);		// Ã¹¹ÙÀÌÆ® ¼Û½Å ½ÃÀÛ
;     691 }
;     692 
;     693 
;     694 //------------------------------------------------------------------------------
;     695 // Flash¿¡¼­ ¾À Á¤º¸ ÀÐ±â
;     696 //	ScIdx : ¾À ÀÎµ¦½º
;     697 //------------------------------------------------------------------------------
...
;     700 	WORD i;
;     701 	
;     702 	Scene.NumOfFrame = gpFN_Table[gSIdx];	// ÇÁ·¹ÀÓ¼ö
;	i -> R16,R17
;     703 	Scene.RTime = gpRT_Table[gSIdx];		// ¾À ¼öÇà ½Ã°£[msec]
;     704 	for(i=0;i<Motion.NumOfwCK;i++){			// °¢ wCK µ¥ÀÌÅÍ ÃÊ±âÈ­
...
;     710 	}
;     711 	for(i=0;i<Motion.NumOfwCK;i++){			// °¢ wCK µ¥ÀÌÅÍ ÀúÀå
...
;     717 	}
;     718 	UCSR0B &= 0x7F;   		// UART0 RxÀÎÅÍ·´Æ® ¹Ì»ç¿ë
;     719 	UCSR0B |= 0x40;   		// UART0 TxÀÎÅÍ·´Æ® »ç¿ë
;     720 
;     721 	delay_us(1300);
;     722 }
;     723 
...
;     725 //------------------------------------------------------------------------------
;     726 // ÇÁ·¹ÀÓ ¼Û½Å °£°Ý °è»ê
;     727 // 		: ¾À ¼öÇà ½Ã°£À» ÇÁ·¹ÀÓ¼ö·Î ³ª´²¼­ intervalÀ» Á¤ÇÑ´Ù
;     728 //------------------------------------------------------------------------------
...
;     746 		TCNT1=65535;
;     747 
;     748 	TIFR |= 0x04;		// Å¸ÀÌ¸Ó ÀÎÅÍ·´Æ® ÇÃ·¡±× ÃÊ±âÈ­
;     749 	TIMSK |= 0x04;		// Timer1 Overflow Interrupt È°¼ºÈ­(140ÂÊ)
;     750 }
;     751 
;     752 
;     753 //------------------------------------------------------------------------------
;     754 // ÇÁ·¹ÀÓ´ç ´ÜÀ§ ÀÌµ¿·® °è»ê
;     755 //------------------------------------------------------------------------------
...
;	i -> R16,R17
;     761 		if(Scene.wCK[i].Exist){	// Á¸ÀçÇÏ´Â ID¸¸ ÁØºñ
;     762 			if(Scene.wCK[i].SPos!=Scene.wCK[i].DPos){
;     763 				// ÇÁ·¹ÀÓ´ç ´ÜÀ§ º¯À§ Áõ°¡·® °è»ê
...
;     771 		}
;     772 	}
;     773 	gFrameIdx=0;				// ÇÁ·¹ÀÓ ÀÎµ¦½º ÃÊ±âÈ­
;     774 }
;     775 
;     776 
;     777 //------------------------------------------------------------------------------
;     778 // ÇÑ ÇÁ·¹ÀÓ ¼Û½Å ÁØºñ
;     779 //------------------------------------------------------------------------------
;     780 void MakeFrame(void)
;     781 {
_MakeFrame:
;     782 	while(gTx0Cnt);			// ÀÌÀü ÇÁ·¹ÀÓ ¼Û½ÅÀÌ ³¡³¯ ¶§±îÁö ´ë±â
_0x68:
	TST  R11
	BRNE PC+3
	JMP _0x6A
	RJMP _0x68
_0x6A:
;     783 	gFrameIdx++;			// ÇÁ·¹ÀÓ ÀÎµ¦½º Áõ°¡
	LDS  R30,_gFrameIdx
	LDS  R31,_gFrameIdx+1
	ADIW R30,1
	STS  _gFrameIdx,R30
	STS  _gFrameIdx+1,R31
;     784 	SyncPosSend();			// µ¿±âÈ­ À§Ä¡ ¸í·ÉÀ¸·Î ¼Û½Å
	CALL _SyncPosSend
;     785 }
	RET
;     786 
...
;     792 {
_SendFrame:
;     793 	if(gTx0Cnt==0)	return;	// º¸³¾ µ¥ÀÌÅÍ°¡ ¾øÀ¸¸é ½ÇÇà ¾ÈÇÔ
	TST  R11
	BREQ PC+3
	JMP _0x6B
	RET
...
_0x6B:
	INC  R13
;     795 	sciTx0Data(gTx0Buf[gTx0BufIdx-1]);		// Ã¹¹ÙÀÌÆ® ¼Û½Å ½ÃÀÛ
	MOV  R30,R13
	SUBI R30,LOW(1)
	LDI  R31,0
	SUBI R30,LOW(-_gTx0Buf)
...
;     805 	WORD i;
;     806 
;     807 	GetMotionFromFlash();		// °¢ wCK ÆÄ¶ó¹ÌÅÍ ºÒ·¯¿À±â
;	tmp -> Y+2
;	i -> R16,R17
;     808 	SendTGain();				// RuntimeÀÌµæ ¼Û½Å
;     809 	for(i=0;i<Motion.NumOfScene;i++){
;     810 		gSIdx = i;
;     811 		GetSceneFromFlash();	// ÇÑ ¾ÀÀ» ºÒ·¯¿Â´Ù
;     812 		SendExPortD();			// È®Àå Æ÷Æ®°ª ¼Û½Å
;     813 		CalcFrameInterval();	// ÇÁ·¹ÀÓ ¼Û½Å °£°Ý °è»ê, Å¸ÀÌ¸Ó1 ½ÃÀÛ
;     814 		CalcUnitMove();			// ÇÁ·¹ÀÓ´ç ´ÜÀ§ ÀÌµ¿·® °è»ê
;     815 		MakeFrame();			// ÇÑ ÇÁ·¹ÀÓ ÁØºñ
;     816 		SendFrame();			// ÇÑ ÇÁ·¹ÀÓ ¼Û½Å
;     817 		while(F_PLAYING);
;     818 	}
;     819 }
;     820 
;     821 
;     822 void SampleMotion1(void)	// »ùÇÃ ¸ð¼Ç 1
;     823 {
...
;     835 }
;     836 //==============================================================================
;     837 //						Digital Input Output °ü·Ã ÇÔ¼öµé
;     838 //==============================================================================
;     839 
...
;	lbtmp -> R18
;     855 	if((lbtmp!=0x03)){
;     856 		if(++gBtnCnt>100){   // ´­·¯¼­ 0.1ÃÊ ÀÌ»ó À¯ÁöµÇ¸é ÀÔ·Â ÀÎÁ¤
;     857 			if(lbtmp==0x02){	// PF1 ¹öÆ° ´­·¯Á³À¸¸é »ùÇÃ ¸ð¼Ç ½ÇÇà
;     858 				SampleMotion1();
;     859 			}
;     860 		}
;     861 	}
...
;     872 {
;     873 	// ¸ðµå Ç¥½Ã LED Ã³¸®
;     874 	if(F_DIRECT_C_EN){		// Á÷Á¢ Á¦¾î ¸ðµåÀÌ¸é
;     875 		PF1_LED1_ON;
;     876 		PF1_LED2_OFF;
;     877 		PF2_LED_ON;
;     878 		return;
