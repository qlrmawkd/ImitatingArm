...
void main(void) {
    ...
    SendSetCmd(1, ID_SET, 10, 10);  // ID 1번 wCk의 ID를 10으로 변경
	...
	while(1){
		...
		while(lMSEC==gMSEC);
		*/
		PositionMove(1, 1, 200);        // 토크1, ID1, 목표위치 200 
		IOwrite( 1, 0);                 // ID 1, 0번 LED ON
		PWR_LED2_ON;    delay_ms(500);                          
		PWR_LED2_OFF;    delay_ms(500);                             		

		PositionMove(1, 1, 50);        // 토크1, ID1, 목표위치 200 
		IOwrite( 1, 3);                 // ID 1, 0번, 1번 LED ON
...
//------------------------------------------------------------------------------
// 8bit 명령 Position Move를 수행하기 위한 함수
