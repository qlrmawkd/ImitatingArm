...
;     422 
;     423 //------------------------------------------------------------------------------
;     424 // 시리얼 포트로 한 문자를 전송하기 위한 함수
;     425 //------------------------------------------------------------------------------
;     426 void sciTx0Data(BYTE td)
;     427 {
_sciTx0Data:
;     428 	while(!(UCSR0A&(1<<UDRE))); 	// 이전의 전송이 완료될때까지 대기
;	td -> Y+0
_0x19:
	SBIS 0xB,5
	RJMP _0x19
...
;     433 {
;     434 	while(!(UCSR1A&(1<<UDRE))); 	// 이전의 전송이 완료될때까지 대기
;	td -> Y+0
;     435 	UDR1=td;
;     436 }
;     437 
;     438 
;     439 //------------------------------------------------------------------------------
;     440 // 시리얼 포트로 한 문자를 받을때까지 대기하기 위한 함수
;     441 //------------------------------------------------------------------------------
...
;     445 	startT = gMSEC;
;	startT -> R16,R17
;     446 	while(!(UCSR0A&(1<<RXC)) ){ 	// 한 문자가 수신될때까지 대기
;     447         if(gMSEC<startT){
;     448 			// 타임 아웃시 로컬 탈출
...
;     459 	startT = gMSEC;
;	startT -> R16,R17
;     460 	while(!(UCSR1A&(1<<RXC)) ){ 	// 한 문자가 수신될때까지 대기
;     461         if(gMSEC<startT){
;     462 			// 타임 아웃시 로컬 탈출
...
;     483 	gTx0Cnt++;			// 송신할 바이트수 증가
;     484 
;     485 	gTx0Buf[gTx0Cnt]=ID;
;     486 	gTx0Cnt++;			// 송신할 바이트수 증가
;     487 
;     488 	gTx0Buf[gTx0Cnt]=Data1;
;     489 	gTx0Cnt++;			// 송신할 바이트수 증가
;     490 
;     491 	gTx0Buf[gTx0Cnt]=Data2;
;     492 	gTx0Cnt++;			// 송신할 바이트수 증가
;     493 
;     494 	gTx0Buf[gTx0Cnt]=Data3;
;     495 	gTx0Cnt++;			// 송신할 바이트수 증가
;     496 
;     497 	gTx0Buf[gTx0Cnt]=CheckSum;
;     498 	gTx0Cnt++;			// 송신할 바이트수 증가
;     499 }
;     500 
;     501 
;     502 //------------------------------------------------------------------------------
;     503 // 동기화 위치 명령(Synchronized Position Send Command)을 보내는 함수
;     504 //------------------------------------------------------------------------------
...
;     513 	gTx0Buf[gTx0Cnt]=HEADER;
	...
	LDI  R30,LOW(255)
	ST   X,R30
;     514 	gTx0Cnt++;			// 송신할 바이트수 증가
	INC  R11
;     515 
;     516 	gTx0Buf[gTx0Cnt]=Data;
	CALL SUBOPT_0x4
	ST   X,R21
;     517 	gTx0Cnt++;			// 송신할 바이트수 증가
	INC  R11
;     518 
;     519 	gTx0Buf[gTx0Cnt]=16;
	...
	LDI  R30,LOW(16)
	ST   X,R30
;     520 	gTx0Cnt++;			// 송신할 바이트수 증가
	INC  R11
;     521 
;     522 	CheckSum = 0;
	LDI  R18,LOW(0)
;     523 	for(i=0;i<MAX_wCK;i++){	// 각 wCK 데이터 준비
	LDI  R19,LOW(0)
_0x2E:
	CPI  R19,31
	BRLO PC+3
	JMP _0x2F
;     524 		if(Scene.wCK[i].Exist){	// 존재하는 ID만 준비
	LDI  R30,LOW(5)
	MUL  R30,R19
	MOVW R30,R0
...
;     539 
;     540 //------------------------------------------------------------------------------
;     541 // 8bit 명령 Position Move를 실행하기 위한 함수
;     542 // Input	: torq ID, Position
;     543 // Output	: None
;     544 //------------------------------------------------------------------------------
...
;     556 
;     557 //------------------------------------------------------------------------------
;     558 // 확장 명령 I/O Write를 수행하기 위한 함수
;     559 // Input	: ID, IOchannel
;     560 // Output	: None
;     561 //------------------------------------------------------------------------------
;     562 void IOwrite(BYTE ID, BYTE IOchannel)
;     563 {
_IOwrite:
;     564     BYTE CheckSum; 
;     565 	ID=(BYTE)(7<<5)|ID; 
...
;	CheckSum -> R16
	...
	ORI  R30,LOW(0xE0)
	STD  Y+2,R30
;     566 	CheckSum = (ID^100^IOchannel^IOchannel)&0x7f;
	LDD  R26,Y+2
	LDI  R30,LOW(100)
	EOR  R30,R26
	LDD  R26,Y+1
	...
	MOV  R16,R30
;     567 
;     568 	gTx0Buf[gTx0Cnt]=HEADER;    gTx0Cnt++;			// 송신할 바이트수 증가
	CALL SUBOPT_0x4
	LDI  R30,LOW(255)
	ST   X,R30
	CALL SUBOPT_0x5
;     569 	gTx0Buf[gTx0Cnt]=ID;        gTx0Cnt++;			// 송신할 바이트수 증가
	LDD  R26,Y+2
	STD  Z+0,R26
	INC  R11
;     570 	gTx0Buf[gTx0Cnt]=100;       gTx0Cnt++;			// 송신할 바이트수 증가
	CALL SUBOPT_0x4
	LDI  R30,LOW(100)
	ST   X,R30
	CALL SUBOPT_0x5
;     571 	gTx0Buf[gTx0Cnt]=IOchannel;	gTx0Cnt++;			// 송신할 바이트수 증가
	LDD  R26,Y+1
	STD  Z+0,R26
	CALL SUBOPT_0x5
;     572 	gTx0Buf[gTx0Cnt]=IOchannel;	gTx0Cnt++;			// 송신할 바이트수 증가
	LDD  R26,Y+1
	STD  Z+0,R26
	INC  R11
;     573 	gTx0Buf[gTx0Cnt]=CheckSum; 	gTx0Cnt++;			// 송신할 바이트수 증가   
	CALL SUBOPT_0x4
	ST   X,R16
	INC  R11
;     574 	
;     575 	gTx0BufIdx++;
	INC  R13
;     576 	sciTx0Data(gTx0Buf[gTx0BufIdx-1]);		// 첫바이트 송신 시작
	MOV  R30,R13
	SUBI R30,LOW(1)
	CALL SUBOPT_0x0
	LD   R30,Z
...
;     579 
;     580 //------------------------------------------------------------------------------
;     581 // 위치 읽기 명령(Position Send Command)을 보내는 함수
;     582 // Input	: ID, SpeedLevel, Position
;     583 // Output	: Current
;     584 // UART0 RX 인터럽트, Timer0 인터럽트가 활성화 되어 있어야 함
;     585 //------------------------------------------------------------------------------
;     586 WORD PosRead(BYTE ID) 
;     587 {
;     588 	BYTE	Data1, Data2;
;     589 	BYTE	CheckSum, Load, Position; 
;     590 	WORD	startT;
;     591 
;     592 	Data1 = (5<<5) | ID;
;	ID -> Y+7
...
;	startT -> Y+5
;     593 	Data2 = 0;
;     594 	gRx0Cnt = 0;			// 수신 바이트 수 초기화
;     595 	CheckSum = (Data1^Data2)&0x7f;
;     596 	sciTx0Data(HEADER);
;     597 	sciTx0Data(Data1);
;     598 	sciTx0Data(Data2);
;     599 	sciTx0Data(CheckSum);
;     600 	startT = gMSEC;
;     601 	while(gRx0Cnt<2){
;     602         if(gMSEC<startT){ 	// 밀리초 카운트가 리셋된 경우
;     603             if((1000 - startT + gMSEC)>RX_T_OUT)
;     604             	return 444;	// 타임아웃시 로컬 탈출
;     605         }
;     606 		else if((gMSEC-startT)>RX_T_OUT) return 444;
;     607 	}
;     608 	return gRx0Buf[RX0_BUF_SIZE-1];
;     609 } 
;     610 
;     611 
;     612 //------------------------------------------------------------------------------
;     613 // Flash에서 모션 정보 읽기
;     614 //	MRIdx : 모션 상대 인덱스
;     615 //------------------------------------------------------------------------------
;     616 void GetMotionFromFlash(void)
;     617 {
;     618 	WORD i;
;     619 
;     620 	for(i=0;i<MAX_wCK;i++){				// wCK 파라미터 구조체 초기화
;	i -> R16,R17
;     621 		Motion.wCK[i].Exist		= 0;
;     622 		Motion.wCK[i].RPgain	= 0;
;     623 		Motion.wCK[i].RDgain	= 0;
;     624 		Motion.wCK[i].RIgain	= 0;
;     625 		Motion.wCK[i].PortEn	= 0;
;     626 		Motion.wCK[i].InitPos	= 0;
;     627 	}
;     628 	for(i=0;i<Motion.NumOfwCK;i++){		// 각 wCK 파라미터 불러오기
;     629 		Motion.wCK[wCK_IDs[i]].Exist		= 1;
;     630 		Motion.wCK[wCK_IDs[i]].RPgain	= *(gpPg_Table+i);
;     631 		Motion.wCK[wCK_IDs[i]].RDgain	= *(gpDg_Table+i);
;     632 		Motion.wCK[wCK_IDs[i]].RIgain	= *(gpIg_Table+i);
;     633 		Motion.wCK[wCK_IDs[i]].PortEn	= 1;
;     634 		Motion.wCK[wCK_IDs[i]].InitPos	= MotionZeroPos[i];
;     635 	}
;     636 }
;     637 
;     638 
;     639 //------------------------------------------------------------------------------
;     640 // Runtime P,D,I 이득 송신
;     641 // 		: 모션정보에서 Runtime P,D,I이득을 불러와서 wCK에게 보낸다
;     642 //------------------------------------------------------------------------------
;     643 void SendTGain(void)
;     644 {
;     645 	WORD i;
;     646 
;     647 	UCSR0B &= 0x7F;   		// UART0 Rx인터럽트 미사용
;	i -> R16,R17
;     648 	UCSR0B |= 0x40;   		// UART0 Tx인터럽트 사용
;     649 
;     650 	while(gTx0Cnt);			// 이전 패킷 송신이 끝날 때까지 대기
;     651 	for(i=0;i<MAX_wCK;i++){					// Runtime P,D이득 설정 패킷 준비
;     652 		if(Motion.wCK[i].Exist)				// 존재하는 ID만 준비
;     653 			SendSetCmd(i, 11, Motion.wCK[i].RPgain, Motion.wCK[i].RDgain);
;     654 	}
;     655 	gTx0BufIdx++;
;     656 	sciTx0Data(gTx0Buf[gTx0BufIdx-1]);		// 첫바이트 송신 시작
;     657 
;     658 
;     659 	while(gTx0Cnt);			// 이전 패킷 송신이 끝날 때까지 대기
;     660 	for(i=0;i<MAX_wCK;i++){					// Runtime I이득 설정 패킷 준비
;     661 		if(Motion.wCK[i].Exist)				// 존재하는 ID만 준비
;     662 			SendSetCmd(i, 24, Motion.wCK[i].RIgain, Motion.wCK[i].RIgain);
;     663 	}
;     664 	gTx0BufIdx++;
;     665 	sciTx0Data(gTx0Buf[gTx0BufIdx-1]);		// 첫바이트 송신 시작
;     666 }
;     667 
;     668 
;     669 //------------------------------------------------------------------------------
;     670 // 확장 포트값 송신
;     671 // 		: 씬 정보에서 확장 포트값을 불러와서 wCK에게 보낸다
;     672 //------------------------------------------------------------------------------
;     673 void SendExPortD(void)
;     674 {
;     675 	WORD i;
;     676 
;     677 	UCSR0B &= 0x7F;   		// UART0 Rx인터럽트 미사용
;	i -> R16,R17
;     678 	UCSR0B |= 0x40;   		// UART0 Tx인터럽트 사용
;     679 
;     680 	while(gTx0Cnt);			// 이전 패킷 송신이 끝날 때까지 대기
;     681 	for(i=0;i<MAX_wCK;i++){					// Runtime P,D이득 설정 패킷 준비
;     682 		if(Scene.wCK[i].Exist)				// 존재하는 ID만 준비
;     683 			SendSetCmd(i, 100, Scene.wCK[i].ExPortD, Scene.wCK[i].ExPortD);
;     684 	}
;     685 	gTx0BufIdx++;
;     686 	sciTx0Data(gTx0Buf[gTx0BufIdx-1]);		// 첫바이트 송신 시작
;     687 }
;     688 
;     689 
;     690 //------------------------------------------------------------------------------
...
;     692 //	ScIdx : 씬 인덱스
;     693 //------------------------------------------------------------------------------
;     694 void GetSceneFromFlash(void)
;     695 {
;     696 	WORD i;
;     697 	
;     698 	Scene.NumOfFrame = gpFN_Table[gSIdx];	// 프레임수
;	i -> R16,R17
;     699 	Scene.RTime = gpRT_Table[gSIdx];		// 씬 수행 시간[msec]
;     700 	for(i=0;i<Motion.NumOfwCK;i++){			// 각 wCK 데이터 초기화
;     701 		Scene.wCK[i].Exist		= 0;
;     702 		Scene.wCK[i].SPos		= 0;
;     703 		Scene.wCK[i].DPos		= 0;
;     704 		Scene.wCK[i].Torq		= 0;
;     705 		Scene.wCK[i].ExPortD	= 0;
;     706 	}
;     707 	for(i=0;i<Motion.NumOfwCK;i++){			// 각 wCK 데이터 저장
;     708 		Scene.wCK[wCK_IDs[i]].Exist		= 1;
;     709 		Scene.wCK[wCK_IDs[i]].SPos		= gpPos_Table[Motion.NumOfwCK*gSIdx+i];
;     710 		Scene.wCK[wCK_IDs[i]].DPos		= gpPos_Table[Motion.NumOfwCK*(gSIdx+1)+i];
;     711 		Scene.wCK[wCK_IDs[i]].Torq		= gpT_Table[Motion.NumOfwCK*gSIdx+i];
;     712 		Scene.wCK[wCK_IDs[i]].ExPortD	= gpE_Table[Motion.NumOfwCK*gSIdx+i];
;     713 	}
;     714 	UCSR0B &= 0x7F;   		// UART0 Rx인터럽트 미사용
;     715 	UCSR0B |= 0x40;   		// UART0 Tx인터럽트 사용
;     716 
;     717 	delay_us(1300);
;     718 }
;     719 
...
;     721 //------------------------------------------------------------------------------
;     722 // 프레임 송신 간격 계산
;     723 // 		: 씬 수행 시간을 프레임수로 나눠서 interval을 정한다
;     724 //------------------------------------------------------------------------------
;     725 void CalcFrameInterval(void)
;     726 {
;     727 	float tmp;
;     728 	if((Scene.RTime / Scene.NumOfFrame)<20){
;	tmp -> Y+0
;     729 		return;
;     730 	}
;     731 	tmp = (float)Scene.RTime * 14.4;
;     732 	tmp = tmp  / (float)Scene.NumOfFrame;
;     733 	TxInterval = 65535 - (WORD)tmp - 43;
;     734 
;     735 	RUN_LED1_ON;
;     736 	F_PLAYING=1;		// 모션 실행중 표시
;     737 	TCCR1B=0x05;
;     738 
;     739 	if(TxInterval<=65509)	
;     740 		TCNT1=TxInterval+26;
;     741 	else
;     742 		TCNT1=65535;
;     743 
;     744 	TIFR |= 0x04;		// 타이머 인터럽트 플래그 초기화
;     745 	TIMSK |= 0x04;		// Timer1 Overflow Interrupt 활성화(140쪽)
;     746 }
;     747 
;     748 
;     749 //------------------------------------------------------------------------------
;     750 // 프레임당 단위 이동량 계산
;     751 //------------------------------------------------------------------------------
;     752 void CalcUnitMove(void)
;     753 {
;     754 	WORD i;
;     755 
;     756 	for(i=0;i<MAX_wCK;i++){
;	i -> R16,R17
;     757 		if(Scene.wCK[i].Exist){	// 존재하는 ID만 준비
;     758 			if(Scene.wCK[i].SPos!=Scene.wCK[i].DPos){
;     759 				// 프레임당 단위 변위 증가량 계산
;     760 				gUnitD[i] = (float)((int)Scene.wCK[i].DPos-(int)Scene.wCK[i].SPos);
;     761 				gUnitD[i] = (float)(gUnitD[i]/Scene.NumOfFrame);
;     762 				if(gUnitD[i]>253)	gUnitD[i]=254;
;     763 				else if(gUnitD[i]<-253)	gUnitD[i]=-254;
;     764 			}
;     765 			else
;     766 				gUnitD[i] = 0;
;     767 		}
;     768 	}
;     769 	gFrameIdx=0;				// 프레임 인덱스 초기화
;     770 }
;     771 
;     772 
;     773 //------------------------------------------------------------------------------
;     774 // 한 프레임 송신 준비
;     775 //------------------------------------------------------------------------------
;     776 void MakeFrame(void)
;     777 {
_MakeFrame:
;     778 	while(gTx0Cnt);			// 이전 프레임 송신이 끝날 때까지 대기
_0x68:
	TST  R11
	BRNE _0x68
;     779 	gFrameIdx++;			// 프레임 인덱스 증가
	LDS  R30,_gFrameIdx
	LDS  R31,_gFrameIdx+1
	ADIW R30,1
	STS  _gFrameIdx,R30
	STS  _gFrameIdx+1,R31
;     780 	SyncPosSend();			// 동기화 위치 명령으로 송신
	CALL _SyncPosSend
;     781 }
	RET
;     782 
...
;     785 // 한 프레임 송신
;     786 //------------------------------------------------------------------------------
;     787 void SendFrame(void)
;     788 {
_SendFrame:
;     789 	if(gTx0Cnt==0)	return;	// 보낼 데이터가 없으면 실행 안함
	TST  R11
	BRNE _0x6B
	RET
;     790 	gTx0BufIdx++;
_0x6B:
	INC  R13
;     791 	sciTx0Data(gTx0Buf[gTx0BufIdx-1]);		// 첫바이트 송신 시작
	MOV  R30,R13
	SUBI R30,LOW(1)
	CALL SUBOPT_0x0
	LD   R30,Z
...
;     796 // 
;     797 //------------------------------------------------------------------------------
;     798 void M_PlayFlash(void)
;     799 {
;     800 	float tmp;
;     801 	WORD i;
;     802 
;     803 	GetMotionFromFlash();		// 각 wCK 파라미터 불러오기
;	tmp -> Y+2
;	i -> R16,R17
;     804 	SendTGain();				// Runtime이득 송신
;     805 	for(i=0;i<Motion.NumOfScene;i++){
;     806 		gSIdx = i;
;     807 		GetSceneFromFlash();	// 한 씬을 불러온다
;     808 		SendExPortD();			// 확장 포트값 송신
;     809 		CalcFrameInterval();	// 프레임 송신 간격 계산, 타이머1 시작
;     810 		CalcUnitMove();			// 프레임당 단위 이동량 계산
;     811 		MakeFrame();			// 한 프레임 준비
;     812 		SendFrame();			// 한 프레임 송신
;     813 		while(F_PLAYING);
;     814 	}
;     815 }
;     816 
;     817 
;     818 void SampleMotion1(void)	// 샘플 모션 1
;     819 {
;     820 	gpT_Table			= M_EX1_Torque;
;     821 	gpE_Table			= M_EX1_Port;
;     822 	gpPg_Table 			= M_EX1_RuntimePGain;
;     823 	gpDg_Table 			= M_EX1_RuntimeDGain;
;     824 	gpIg_Table 			= M_EX1_RuntimeIGain;
;     825 	gpFN_Table			= M_EX1_Frames;
;     826 	gpRT_Table			= M_EX1_TrTime;
;     827 	gpPos_Table			= M_EX1_Position;
;     828 	Motion.NumOfScene 	= M_EX1_NUM_OF_SCENES;
;     829 	Motion.NumOfwCK 	= M_EX1_NUM_OF_WCKS;
;     830 	M_PlayFlash();
;     831 }
;     832 //==============================================================================
;     833 //						Digital Input Output 관련 함수들
;     834 //==============================================================================
;     835 
...
;	lbtmp -> R18
;     851 	if((lbtmp!=0x03)){
;     852 		if(++gBtnCnt>100){   // 눌러서 0.1초 이상 유지되면 입력 인정
;     853 			if(lbtmp==0x02){	// PF1 버튼 눌러졌으면 샘플 모션 실행
;     854 				SampleMotion1();
;     855 			}
;     856 		}
;     857 	}
...
;     865 // Io 업데이트 처리
;     866 //------------------------------------------------------------------------------
;     867 void IoUpdate(void)
;     868 {
;     869 	// 모드 표시 LED 처리
;     870 	if(F_DIRECT_C_EN){		// 직접 제어 모드이면
;     871 		PF1_LED1_ON;
;     872 		PF1_LED2_OFF;
;     873 		PF2_LED_ON;
;     874 		return;
